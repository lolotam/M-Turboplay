...
const BestSellers = () => {
  ...
  // Filter products for "Best Sellers" - products with high ratings and good sales
  const bestSellerProducts = useMemo(() => {
    return products.filter(product =>
      product.status === 'active' &&
      (product.isLimited || product.isNew || Math.random() > 0.7) // Mock best seller criteria
    );
  }, [products]);

  // Get unique categories from best seller products
  ...
  const filteredAndSortedProducts = useMemo(() => {
    let filtered = bestSellerProducts;
    ...
    const sorted = [...filtered].sort((a, b) => {
      switch (sortBy) {
        ...
        case 'popularity':
        default:
          // Sort by a combination of rating and limited/new status
          const scoreA = (a.rating || 4.8) + (a.isLimited ? 0.5 : 0) + (a.isNew ? 0.3 : 0);
          const scoreB = (b.rating || 4.8) + (b.isLimited ? 0.5 : 0) + (b.isNew ? 0.3 : 0);
          return scoreB - scoreA;
      }
    });

    return sorted;
  }, [bestSellerProducts, searchQuery, selectedCategory, sortBy, isRTL]);
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-4">
            <Badge className="bg-[#F59E0B] text-white px-3 py-1">
          ...
          <p className="text-muted-foreground text-lg">
            {isRTL
              ? 'اكتشف الألعاب الأكثر شعبية ومبيعاً في متجرنا'
              : 'Discover the most popular and best-selling games in our store'
            }
          </p>
        </div>
        ...
        <div className="bg-card/50 rounded-xl p-6 mb-8">
          <div className="flex flex-col lg:flex-row gap-4 items-center justify-between">
            ...
            <div className="relative flex-1 max-w-md">
              ...
              <Input
                placeholder={isRTL ? 'البحث في الألعاب الأكثر مبيعاً...' : 'Search best sellers...'}
                value={searchQuery}
          ...
          <div className="mt-4 text-sm text-muted-foreground">
            {isRTL
              ? `عرض ${paginatedProducts.length} من ${filteredAndSortedProducts.length} منتج أكثر مبيعاً`
              : `Showing ${paginatedProducts.length} of ${filteredAndSortedProducts.length} best-selling products`
            }
          </div>
        </div>
        ...
        {paginatedProducts.length > 0 ? (
            ...
            <div className={`grid gap-6 ${
              ...
              {paginatedProducts.map((product) => (
                <ProductCard
                  ...
                  rating={4.9} // High rating for best sellers
                  isNew={product.isNew}
          ...
          <div className="text-center py-16">
            ...
            <h3 className="text-xl font-semibold mb-2">
              {isRTL ? 'لا توجد منتجات أكثر مبيعاً حالياً' : 'No Best Sellers Found'}
            </h3>
            <p className="text-muted-foreground mb-6">
              {isRTL
                ? 'لا توجد منتجات أكثر مبيعاً تطابق معايير البحث الحالية'
                : 'No best-selling products match your current search criteria'
              }
            </p>
            <Button
              variant="outline"
